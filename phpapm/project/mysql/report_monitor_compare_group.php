        ...
        foreach ($arr_add as $v) {
            if ($v != '') {
                ...
                $sql = "insert into ".APM_DB_PREFIX."monitor_v1
                        (V1,COUNT_TYPE,CHAR_TYPE,START_CLOCK,SHOW_ALL,
                        ID,DAY_COUNT_TYPE,HOUR_COUNT_TYPE,PERCENT_COUNT_TYPE,SHOW_AVG)
                        values(:V1,:COUNT_TYPE,:CHAR_TYPE,:START_CLOCK,:SHOW_ALL,
                        NULL,:DAY_COUNT_TYPE,:HOUR_COUNT_TYPE,:PERCENT_COUNT_TYPE,:SHOW_AVG)";
                $stmt = apm_db_parse($conn_db, $sql);
                apm_db_bind_by_name($stmt, ':V1', $v);
