...
class sysload
{
    function _initialize()
    {
        ...
        print_r($out);
        _status(round($out[1], 2), APM_HOST . '(WEB日志分析)', 'Load', $_POST['hostname'], $replace_date, APM_HOSTNAME, 0, 'replace');
        ...
        echo "IO\n";
        print_r($io);
        _status($io, APM_HOST . '(WEB日志分析)', 'IO', $_POST['hostname'], date('Y-m-d H'), APM_HOSTNAME, 0, 'replace');

        //运行时间
        preg_match('#up ([0-9]+) day#iUs', $uptime[0], $out);
        echo "运行时间\n";
        print_r($out);
        _status($out[1], APM_HOST . '(WEB日志分析)', '运行天数', $_POST['hostname'], date('Y-m-d H'), APM_HOSTNAME, 0, 'replace');

        //监控内存剩余
        exec("cat /proc/meminfo | head -2 | tail -1", $mem); //mem
        print_r($mem);
        preg_match('#.*([0-9]+) KB#iUs', $mem[0], $out);
        _status(round($out[1] / 1024, 2), APM_HOST . '(WEB日志分析)', 'Mem内存剩余', $_POST['hostname'], $replace_date, APM_HOSTNAME, 0, 'replace');
        ...
        _status($_POST['cpu'], APM_HOST . '(WEB日志分析)', 'CPU', $_POST['hostname'], $_POST['cpu'] . '%-' . $replace_date, APM_HOSTNAME, 0, 'replace');
        ...
        foreach ($disk as $row) {
            ...
            $num = $tmp[1];
            $num = str_replace('%', '', $num);
            _status($num, APM_HOST . '(WEB日志分析)', '磁盘', $_POST['hostname'] . '-' . $mnt_name, $row . '-' . $replace_date, APM_HOSTNAME, 0, 'replace');
        }
