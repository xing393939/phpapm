...
class P1D_ClickStats
{
    ...
    function _get_stats_flag_data($conn_db, $v1, $v2, $stats_flag, $date)
    {
        ...
        //更新配置表
        $sql = "update ".APM_DB_PREFIX."monitor set fun_count=:fun_count,oci_unique=".round(lcg_value() * 100000000)." where v1=:v1 and v2=:v2 and cal_date =to_date(:cal_date,'yyyy-mm-dd hh24:mi:ss')";
        $stmt = apm_db_parse($conn_db, $sql);
        ...
        $error = apm_db_execute($stmt);
        if (apm_db_row_count($stmt)) {
            $sql = "insert into ".APM_DB_PREFIX."monitor(id, v1, v2, v3, v5,  fun_count, cal_date, md5, add_time) values
                (NULL, :v1, :v2, :vip, :vip, :fun_count, to_date(:cal_date,'yyyy-mm-dd hh24:mi:ss'), :md5, sysdate)";
            $stmt = apm_db_parse($conn_db, $sql);
            apm_db_bind_by_name($stmt, ':v1', $v1);
            apm_db_bind_by_name($stmt, ':v2', $v2);
        ...
        //更新天表数据
        $sql = "update ".APM_DB_PREFIX."monitor_date set fun_count=:fun_count,oci_unique=".round(lcg_value() * 100000000)." where v1=:v1 and v2=:v2 and cal_date =to_date(:cal_date,'yyyy-mm-dd')";
        $stmt = apm_db_parse($conn_db, $sql);
        apm_db_bind_by_name($stmt, ':v1', $v1);
        apm_db_bind_by_name($stmt, ':v2', $v2);
        apm_db_bind_by_name($stmt, ':cal_date', $date);
        ...
        //更新小时表数据
        $sql = "update ".APM_DB_PREFIX."monitor_hour set fun_count=:fun_count,oci_unique=".round(lcg_value() * 100000000)." where v1=:v1 and v2=:v2 and cal_date =to_date(:cal_date,'yyyy-mm-dd hh24:mi:ss')";
        $stmt = apm_db_parse($conn_db, $sql);
        apm_db_bind_by_name($stmt, ':v1', $v1);
        apm_db_bind_by_name($stmt, ':v2', $v2);
        apm_db_bind_by_name($stmt, ':cal_date', $date);
