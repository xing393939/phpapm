 ...
 * @throws 注意:无DB异常处理
 */
class web_log extends project_config
{
        ...
        if ($this->log_path) {
            ...
            if ($_GET['gz']) {
                for ($i = strtotime(date('Y-m-d H:00:00', strtotime('-1 hours'))); $i < strtotime(date('Y-m-d H:00:00', strtotime('-0 hours'))); $i += 600) {
                    ...
                    _status(1, VHOST . '(BUG错误)', '文件读写', VIP . ADD_PROJECT, "{$gz_dir}*{$i_linux}*@file:" . GET_INCLUDED_FILES . "/{$_GET['act']}", VIP, $diff_time);
                    $qps_stats = max($qps_stats, $this->_web_log("/home/webid/logs/{$logfilename}"));
                }
            } else {
                $logfilename = VHOST . date('_Y_m_d_H', strtotime('-1 hours')) . '_access.log';
                copy($this->log_path . $logfilename, "/home/webid/logs/{$logfilename}");
                $qps_stats = $this->_web_log("/home/webid/logs/{$logfilename}");
            }
            _status($qps_stats, VHOST . '(WEB日志分析)', 'QPS', 'QPS', null, VIP, 0, NULL, strtotime('-1 hours'));
        }

        //js脚本错误记录.
        if ($this->err_log_path && method_exists($this, '_err_weblog')) {
            //web日志
            if ($_GET['gz']) {
                for ($i = strtotime(date('Y-m-d H:00:00', strtotime('-1 hours'))); $i < strtotime(date('Y-m-d H:00:00', strtotime('-0 hours'))); $i += 600) {
                    $gz_dir = $_GET['gz'] . '/' . date('Y-m-d', $i) . '/' . $this->err_log_path;
                    $logfilename = date('Y_m_d_H_i', $i) . $this->err_log_path . '_access.log';
                    $i_linux = substr(strval($i), 0, -1);
                    $tt1 = microtime(true);
                    echo "tar zxvf {$gz_dir}*{$i_linux}* -O >/home/webid/logs/{$logfilename}\n";
                    exec("tar zxvf {$gz_dir}*{$i_linux}* -O >/home/webid/logs/{$logfilename}");
                ...
                copy($this->err_log_path . $logfilename, "/home/webid/logs/{$logfilename}");
                $this->_err_weblog($this->err_log_path . $logfilename);
            }
        }
        ...
        foreach ($arr as $k => $v) {
            if (trim($v) !== '' || $v != 0) {
                if (strpos($v, 'PHP Warning')) {
                    _status(1, VHOST . '(BUG错误)', 'PHP错误', 'PHP错误日志', NULL, VIP);
                } else {
                    $v = substr($v, 22);
                    _status(1, VHOST . '(BUG错误)', 'PHP错误', 'PHP错误日志', $v, VIP);
                }
                if (strpos($v, 'Fatal error'))
                    _status(1, VHOST . '(BUG错误)', '致命错误', 'PHP错误日志', NULL, VIP);
            }
        }
    }
